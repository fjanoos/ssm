...
while(1)
    ...
    %cafe_logo( mfilename, 'messg', ...
	%       sprintf(['\n\n\n%s%s\n* subject : %s  *  iteration# : %d  *  %s\n%s%s\n'], ...
	%	       myLogo.color.C_red, repmat( '=', [1,80]), data.subject_id, it_cnt, datestr( now, 31), repmat( '=', [1,80]), myLogo.color.C_white_n)) ;
...
% $$$     display([ 'relative change in noise = ', num2str(rel_change{it_cnt}.Sigma_eps)]) ; 
    ...
    % evaluate model fit        
    if rel_change{ it_cnt}.error < tol || it_cnt > 15
	break
    end
